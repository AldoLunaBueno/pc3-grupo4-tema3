...
if [[ "$num_steps" -eq 0 ]]; then
    log_info "No se encontraron pasos de construcción en '$CONFIG_FILE'."
    exit 0
fi
...
echo "$build_steps_content" | while IFS= read -r line || [[ -n "$line" ]]; do
    if [[ "$line" =~ ^[[:space:]]*-[[:space:]]+name: ]]; then
        if [[ -n "$current_step_lines" ]]; then
            ...
            if ! $TERRAFORM_CMD init -input=false -no-color -upgrade > /dev/null 2>&1; then
                ...
                echo "$variables_block" | while IFS= read -r var_line || [[ -n "$var_line" ]]; do
                    ...
                    if [[ "$clean_var_line" =~ ^([a-zA-Z0-9_]+):[[:space:]]*(.*)$ ]]; then
                        ...
                        if [[ -z "$current_var_value" ]]; then
                            ...
                            current_var_value_no_quotes=$(echo "$current_var_value" | sed -e "s/^['\"]//" -e "s/['\"]$//")
                        ...
                        sub_value_no_quotes=$(echo "$sub_value" | sed -e "s/^['\"]//" -e "s/['\"]$//")
...
echo
echo "Fin del Proceso de Construcción con Terraform"
exit 0
